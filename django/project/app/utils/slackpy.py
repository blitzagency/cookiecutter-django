...
class SlackLogger:
    ...
    def __build_payload(self, message, title, color, short=False):
    ...
    def __send_notification(self, message, title, color='good', short=False):
        ...
        Raises:
            TODO:
        """
        payload = self.__build_payload(message, title, color, short)
    ...
    def debug(self, message, title='Slack Notification', short=False):
        title = 'DEBUG : {0}'.format(title)
        return self.__send_notification(message=message, title=title, color='#03A9F4', short=short)

    def info(self, message, title='Slack Notification', short=False):
        title = 'INFO : {0}'.format(title)
        return self.__send_notification(message=message, title=title, color='good', short=short)

    def warn(self, message, title='Slack Notification', short=False):
        title = 'WARN : {0}'.format(title)
        return self.__send_notification(message=message, title=title, color='warning', short=short)

    def error(self, message, title='Slack Notification', short=False):
        title = 'ERROR : {0}'.format(title)
        return self.__send_notification(message=message, title=title, color='danger', short=short)

    def warning(self, message, title='Slack Notification', short=False):
        title = 'WARNING : {0}'.format(title)
        return self.__send_notification(message=message, title=title, color='danger', short=short)
