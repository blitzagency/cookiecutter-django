...
def get_heroku_asset_version(env):
    git_hash = local('heroku config:get ASSET_VERSION -a {}'.format(
        APP_INFO[env]["heroku_app_name"]), capture=True)
    print("got hash: ", yellow('{}'.format(git_hash)))
    return git_hash


def set_heroku_asset_version(env, git_hash):
    local('heroku config:set ASSET_VERSION={} -a {}'.format(git_hash,
                                                            APP_INFO[env]["heroku_app_name"]))


def set_heroku_maintenance_page(env, url):
    local('heroku config:set MAINTENANCE_PAGE_URL={} -a {}'.format(url,
                                                                   APP_INFO[env]["heroku_app_name"]))


def set_heroku_error_page(env, url):
    local('heroku config:set ERROR_PAGE_URL={} -a {}'.format(url,
                                                             APP_INFO[env]["heroku_app_name"]))
...
def deploy(env=None, quick=False):
    ...
    version = get_heroku_asset_version(
        env) if quick else current_asset_version(env=env)
    current_branch = local('git rev-parse --abbrev-ref HEAD', capture=True)
    if current_branch != APP_INFO[env]['branch_name']:
        print(red("NOT ON DESIRED DEPLOY BRANCH: {}".format(
            APP_INFO[env]["branch_name"])))
...
def deploy_source(env=None, asset_version='', quick=False, agency_vars={}):
    ...
    local('git push {} {}:master'.format(APP_INFO[env][
          "heroku_remote_name"], APP_INFO[env]["branch_name"]))
...
def clear_cache(env=None, asset_version='', quick=False, agency_vars={}):
    local(
        'heroku run ./manage.py clear_cache -a {}'.format(APP_INFO[env]["heroku_app_name"]))
...
def sync_prod_db(env=None, reset_db=False, agency_vars={}):
    ...
    if reset_db:
        ...
        # local('heroku pg:reset DATABASE_URL') #add "--confirm haus" to remove
        # required input
        pass
    local(
        'heroku run ./manage.py syncdb -a {}'.format(APP_INFO[env]["heroku_app_name"]))
    local(
        'heroku run ./manage.py migrate -a {}'.format(APP_INFO[env]["heroku_app_name"]))
...
def compile_env_css(env=None, asset_version='', agency_vars={}):
    ...
    css_compile()
    log.warning(
        red('Note killall was called so vagrant server/compass will be down'))
